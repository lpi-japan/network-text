...
## ネットワークアドレスとネットマスク
ネットワークアドレスは、ノードが接続されているネットワーク全体を指し示すアドレスです。

インターネットを支えるIPというプロトコルは、複数のノードが集まったネットワーク同士を相互に接続して通信を行う、という考え方に基づいています。外部にある別のネットワークを指し示すのがネットワークアドレスです。

同じネットワークに接続されている各ノードのIPアドレスは、同じ「ネットワーク部」の値と、ノード毎に異なる「ホスト部」の値から成ります。どこまでがネットワーク部かを表すのが「ネットマスク」です。IPアドレスの2進数表記32ビットのうち、ネットマスクで決められた部分まではすべて同じアドレス、そして残りの部分は各ノード毎に異なるアドレス値を取ります。

![ネットワークアドレス](image/Ch02/networkaddress.png){width=70%}

たとえば、「192.168.56.101」というIPアドレスのうち、第3オクテットまでの24ビットがネットワーク部だとした時、「192.168.56.101/24」とIPアドレスの後ろにネットマスクの長さを記述します。この表記を「プレフィックス表記」と呼ぶことがあります。
...
なお、クラスDはマルチキャストアドレス用、クラスEは将来用に予約された範囲となります。
...
## GUIによるIPアドレスの変更
GUIによるIPアドレスの変更は、NetworkManagerに対応した「設定」アプリから行えます。
...
![IPアドレス変更を確認](image/Ch02/config_ipv4_check.png){width=70%}

IPアドレスが指定したIPアドレスに変更されたのが確認できます。
...
TUI操作は慣れが必要なため、まずはTABキーで選択項目を移動させて、Enterキーを押してどのように画面が遷移するのかなどを確認してみてください。
...
enp0s8デバイスにはenp0s8というコネクションが適用されているのがわかります。コネクションがネットワークの各種情報を保持しています。
