# ネットワークインターフェースの管理
ネットワークインターフェースは、Linuxが動作するハードウェアに備えられているネットワーク機能を指します。

Linuxでは、ドライバーとしてカーネルモジュールをロードして使うことができるようになります。通常は各インターフェースにIPアドレスを割り当てて通信を行いますが、複数のインターフェースを束ねて冗長化することで性能や耐障害性を高めることができます。

ネットワークインターフェースの基本的な管理方法と、冗長化の方法について解説します。

## ネットワークインターフェースとは
ネットワークインターフェースの接続方式は、有線LANや無線LAN、物理的な形状としてオンボードにネットワークカード、USB接続など、様々な種類があります。

### オンボード
ハードウェアの中心基板であるマザーボード上に最初から備えられているネットワークインターフェース。
