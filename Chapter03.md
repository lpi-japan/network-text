...
TCPの特徴は、3wayハンドシェイクによるセッションの確立と、シーケンス番号を使った通信の保証にあります。
...
![3wayハンドシェイク](image/Ch03/3way.png){width=70%}

この3つのやり取りで3wayハンドシェイクは完了し、通信のためのセッションが確立します。

### シーケンス番号と通信の保証
3wayハンドシェイクのやり取りでは、TCPの特徴である通信の保証を行うため、お互いのシーケンス番号を交換しています。
...
![シーケンス番号](image/Ch03/sequence.png){width=70%}
...
![ポート番号の役割](image/Ch03/portnumber.png){width=70%}
...
## Registeredポート
ポート番号のうち、1024番から49151番までをRegisteredポートと呼びます。インターネットに関わる様々な事柄を管理する組織であるIANA（Internet Assigned Numbers Authority）に申請を行い、登録される（Registered）ことで他のプロトコルとポート番号が重複しないようにしています。

### ハイポート
プロセスが自由に使えるポート番号です。IANAが管理しているWell-KnownポートとRegisteredポートよりも上の番号なのでハイ（High）ポートと呼ばれます。
