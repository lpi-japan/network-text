# ルーティング/IPマスカレード/プロキシ
ルーティングは、インターネットの基本的な仕組みであるネットワーク間を接続するための仕組みです。遠く離れたネットワーク同士が通信できるのは、ルーティングによってパケットがルーターを経由して適切に転送されるためです。

IPマスカレードは、ローカルネットワークからグローバルネットワークにパケットを送り出す際にアドレスを変換するLinuxの仕組みです。

別々のネットワークを接続する実習として、IPマスカレードの設定を解説します。

また、特定の通信プロトコルを中継する仕組みであるプロキシの設定も解説します。

## ルーティング
ルーティングは、送信側が接続されているネットワークと異なるネットワークにIP通信を行う仕組みです。

ルーターがルーティングを行いますが、受信側のネットワークが直接接続されていない場合、複数のルーターを介してルーティングが行われる場合もあります。
...
## IPマスカレードによるアドレス変換
IPマスカレードは、LinuxでNAPTを行うnftablesの機能です。

NAPTの動作を確認するために、IPマスカレードを設定し、動作させてみましょう。

### システムの構成
クライアント用仮想マシンを別途作成し、実習用の仮想マシンのIPマスカレードを経由してインターネットに接続させます。

クライアント用仮想マシンと実習用の仮想マシンは、VirtualBoxのホストオンリーネットワークで接続します。実習用の仮想マシンはVirtualBoxのNATを経由して、さらにルーターのNAPTを経由してインターネットに接続しているので、多段のアドレス変換を行っていることになります。

![IPマスカレード構成図](image/Ch05/ipmasq.png){width=70%}
...
ネットワークの外側としてexternalゾーン、内側としてinternalゾーンを使用することにします。後ほど確認しますが、externalゾーンにはあらかじめIPマスカレードマスカレードが設定されているので、細かい設定をしないでもIPマスカレードを動作させることができます。
...
デフォルトではinternalゾーンのターゲットがdefaultになっているため、クライアント用仮想マシンから送られてきた転送すべきパケットをすべて落としてしまいます。internalゾーンのターゲットをACCEPTに変更します。
...
![参照DNSとゲートウェイの設定](image/Ch05/config_network.png){width=70%}
...
![参照DNSとゲートウェイの設定を確認](image/Ch05/config_network_check.png){width=70%}
...
![IPマスカレードによる送信元IPアドレス書き換えの確認](image/Ch05/wireshark_dns.png){width=70%}

実行例では、4つのパケットがキャプチャされています。非常に高速な処理な為、Timeの表示が丸められてしまっていますが、リスト表示のTimeをクリックすると時間順にソート表示されます。問い合わせのパケットの送信元が「192.168.56.102」から「10.0.2.15」に書き換えられ、戻って来たパケットの返信先が「10.0.2.15」から「192.168.56.102」に書き換えられているのがわかります。
...
インターフェースの接続がpublicゾーンに戻ったことを確認します。
...
## プロキシの設定
プロキシは、特定の通信プロトコルを中継する仕組みです。主にWebアクセスで利用されており、アクセスログを取得したり、許可されてないWebサイトへのアクセスを禁止したりすることに使われています。

AlmaLinuxではSquidを使うことでプロキシを実行できます。
...
### firewalldの設定変更
クライアント用仮想マシンをSquidに接続させるためには、ポート番号3128への接続を許可する必要があります。firewall-cmdコマンドでfirewalldの設定を変更します。
...
## クライアント用仮想マシンのプロキシ設定
クライアント用仮想マシンのプロキシ設定は、GUIの設定アプリで設定可能です。

クライアント用仮想マシンで「設定」アプリを起動して、「ネットワーク」を選択します。「ネットワーク」プロキシの歯車ボタンをクリックします。
...
![プロキシの設定](image/Ch05/config_proxy.png){width=70%}

プロキシの設定はGUI環境の環境変数として設定されるので、「端末」を起動してenvコマンドで確認します。環境変数「HTTPS_PROXY」、「HTTP_PROXY」、「NO_PROXY」が設定されていることを確認します。環境変数が設定されていない場合、一度「端末」を終了して、再度「端末」を起動してみてください。
...
### リモートログインではプロキシは設定されない
GUIで設定したプロキシ設定は、GUI環境でのみ有効です。そのため、SSHでリモートログインしても環境変数は設定されません。

リモートログインしたシェルでプロキシを設定したい場合には、別途.bashrcなどに同様の環境変数を設定してください。

## プロキシ経由でアクセスする
準備ができたら、クライアント用仮想マシンからプロキシ経由でインターネットのWebサーバーにアクセスします。
...
### プロキシ経由でWebサーバーにアクセス
クライアント用仮想マシンでWebブラウザからインターネット上のサイトにアクセスします。
